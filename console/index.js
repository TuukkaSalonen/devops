...
app.get("/logs", verifyJWT, async (req, res) => {
  try {
    ...
    const logs = await fetch("http://service1_blue:8080/log", {
...
app.get("/status", verifyJWT, async (req, res) => {
  try {
    const token = getParsedToken(req);
    if (!token) return res.status(401).send("Not authenticated");
    const statusResponse = await fetch("http://service1_blue:8080/status", {
      headers: { Authorization: `Bearer ${token}` },
    });
...
app.post("/switch", verifyJWT, (req, res) => {
  try {
    const cmd = `ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i /root/.ssh/id_rsa ubuntu@${TARGET_HOST} "sudo /usr/local/bin/switch_version.sh green"`;
...
// Discard version endpoint (cant discard blue in blue)
app.post("/discard", verifyJWT, (req, res) =>
  res.send("Cannot discard in blue environment")
);
...
app.post("/resetlog", verifyJWT, async (req, res) => {
  try {
    ...
    const deleted = await fetch("http://service1_blue:8080/log", {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
